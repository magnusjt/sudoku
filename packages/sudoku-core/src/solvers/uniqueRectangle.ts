...
export const uniqueRectangle1 = (board: SolverBoard) => {
    for(const points of getAllRows()){
        ...
        for(const pair of pairs){
            ...
            for(const colPair of matchingPairs){
...
export const hiddenRectangle = (board: SolverBoard) => {
    ...
    for(const startingCorner of biValuePoints) {
        ...
        for(const rowCorner of row){
            for(const colCorner of col){
                const x = rowCorner.x
                const y = colCorner.y
