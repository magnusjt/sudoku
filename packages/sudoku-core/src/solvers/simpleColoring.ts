...
function *simpleColoringGenerator(board: SolverBoard){
    ...
    for(const { cand, points } of candsWithPoints){
        // Avoid checking points within the same coloring multiple times
        const checked = new Set()
        for(const startingPoint of points){
            ...
            for(const uncoloredPoint of uncoloredPoints){
            ...
            for(const {point, color} of Object.values(colors)){
